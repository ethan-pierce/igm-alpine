...
for exp in [100, 80, 60, 40, 20]:
    ...
    glacier.config.observation_file = './inputs/observation_' + str(exp) + '_pct_conf.nc'
    ...
    for file in os.listdir('.'):
        if os.path.isfile(file):
            if file not in ['igm-experiments.py', 'igm-experiments.slurm']:
                os.rename(file, os.path.join('./outputs/' + str(exp), file))
